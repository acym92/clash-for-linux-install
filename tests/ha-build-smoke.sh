...
[ "$(grep -c '^mixed-port:' "$_HA_BUILD_FILE")" -eq 1 ]
...
[ "$("$BIN_YQ" '.proxy-groups[] | select(.name == "CODEX-PROBE") | .proxies | length' "$_HA_BUILD_FILE")" = 2 ]
[ "$("$BIN_YQ" -o=json -I=0 '.proxy-groups[] | select(.name == "CODEX") | .proxies' "$_HA_BUILD_FILE")" = '["CODEX-HA","[A] node-one","[B] node-two"]' ]
[ "$("$BIN_YQ" -o=json -I=0 '.listeners[] | select(.name == "codex-ha-probe")' "$_HA_BUILD_FILE")" = '{"name":"codex-ha-probe","type":"http","listen":"127.0.0.1","port":19099,"proxy":"CODEX-PROBE"}' ]
